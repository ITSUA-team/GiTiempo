...
- When extending the `FirebaseAdminService` contract or changing Firebase auth/invite provisioning behavior, update the interface, real provider, fake provider, unit tests, and any relevant e2e coverage in the same change. The fake must remain deterministic and hermetic for `NODE_ENV=test`; do not require real Firebase credentials in tests.
...
- `pnpm --filter @gitiempo/api lint` is check-only for CI parity. Use `pnpm --filter @gitiempo/api lint:fix` for local auto-fixes.
