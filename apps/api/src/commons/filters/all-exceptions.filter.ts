...
interface SanitizedServerErrorLog {
  type: string;
  message: string;
}
...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  private sanitizeServerErrorForLog(
    exception: unknown,
  ): SanitizedServerErrorLog {
    if (exception instanceof Error) {
      return {
        type: exception.name || exception.constructor.name || 'Error',
        message: 'Internal server error',
      };
    }
