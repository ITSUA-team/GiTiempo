...
The backend MUST provide a current-user endpoint that returns the authenticated user's public profile.
...
The backend SHALL allow updates only to mutable current-user profile fields defined by the shared contracts.

#### Scenario: Update display name or avatar

- GIVEN an authenticated user sends a valid current-user update payload
- WHEN the backend processes the request
- THEN the user's mutable profile fields are updated
