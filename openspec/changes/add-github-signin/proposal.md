...
- **Backend** (`apps/api`): new `auth/github` controller + service, `AuthService.createSessionForVerifiedEmail`, `GithubSessionDto`, and `GITHUB_SIGNIN_CLIENT_ID`/`GITHUB_SIGNIN_CLIENT_SECRET` env. The CSRF state is a short-lived, purpose-scoped JWT signed with `JWT_ACCESS_SECRET` (it omits the issuer/audience the access-token verifier requires, so it can never pass as a session token) and is bound to the initiating browser through an HttpOnly nonce cookie. The one-time handoff is an opaque random code whose email is held server-side, so no account data travels in the redirect URL. No database, JWT-contract, or Firebase Admin change.
