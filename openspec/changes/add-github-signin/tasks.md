...
- [x] 1.1 Add `AuthGithubService` (`apps/api/src/auth/services/auth-github.service.ts`): build the authorize URL (dedicated OAuth App client id, `<APP_URL>/auth/github/callback`, `user:email`, signed state), exchange the code, read the primary verified email from `GET /user/emails`, issue a one-time handoff, and exchange the handoff for a session.
- [x] 1.2 Sign the CSRF state as a short-lived JWT (`JWT_ACCESS_SECRET`) carrying a distinct `purpose` and no issuer/audience, so it cannot pass the access-token verifier, and bind it to the initiating browser by signing in the SHA-256 hash of a nonce returned in an HttpOnly `gh_oauth_state` cookie. Make the handoff an opaque random code whose email is held in a short-lived in-memory store and deleted on read, so it is single-use and carries no account data through the redirect URL.
