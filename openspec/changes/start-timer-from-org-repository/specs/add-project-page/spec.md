...
**Reason**: Source is now a field of the Add Project form, and the sidebar card no longer carries selectable tiles. The requirement described inert markup that the page deliberately no longer has.

**Migration**: The behaviour the card's tiles stood for is covered by `Add Project Form Collects Required Fields` and `Add Project Imports An Organization GitHub Project`. The sidebar card remains as explanatory copy about naming and the repository link.
