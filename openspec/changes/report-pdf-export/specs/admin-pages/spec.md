...
- **AND** the `Export` menu is disabled with an explanatory reason while any aggregate-level table filter (search, entries, hours, billable, billable share, or last activity) is active, because the exported file cannot express those filters
