...
- [x] 4.1 Complete reusable-pattern and design-parity review against `GITiempo.pen` screens `Admin Reports`, `Admin Members`, and `Admin Projects`, documenting any PrimeVue-only compromises.
- [x] 4.2 Run `pnpm --filter admin-web lint`.
- [x] 4.3 Run `pnpm --filter admin-web typecheck`.
- [x] 4.4 Run `pnpm --filter admin-web test -- src/components/MembersTable.spec.ts src/components/ProjectsTable.spec.ts src/components/reports/ReportsTable.spec.ts src/lib/report-view-model.spec.ts src/lib/admin-dashboard-view-model.spec.ts`.
