...
- [x] 1.3 Decide how the menu names a running timer, since signing out leaves it running (design D5). This is the one place the design has to prevent a surprise rather than just present two actions. — **Resolved: a one-line warning on the active-status tint.** This went back and forth: a generic sentence first, then the elapsed value and task on the reasoning that the panel had just covered them, then back to a sentence after review — repeating the clock inside a panel sitting over the clock read as a second timer rather than as a warning about the first. The final wording is `Timer keeps running after sign out`.
