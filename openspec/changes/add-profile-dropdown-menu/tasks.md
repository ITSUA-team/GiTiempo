...
- [x] 2.1 Wire `apps/user-web/src/components/layout/AppShell.vue` so `Profile` uses the profile nav icon, navigates to the existing user profile/settings surface, and `Sign out` calls the user auth store logout flow before redirecting to the user login route.
