...
The service therefore reads the current row and decides in JavaScript which write to attempt, but the ownership rule itself is enforced by the database. The avatar is written by its own statement carrying `WHERE id = ... AND avatar_source = 'provider'`, separate from the email and display-name update, which must apply regardless of ownership. `updateFromFirebase` loads the row by id and then issues that guarded write; `upsertFromFirebase` keeps its atomic insert-on-conflict for row existence — so concurrent invite acceptance still cannot produce a duplicate-key failure — and applies the same guarded write to the row the upsert returns.

The guard is not redundant with the JavaScript check, and an earlier revision of this design was wrong to treat it as optional. Deciding in JavaScript and then writing unconditionally leaves a window in which a concurrent `PATCH /users/me` sets a member-chosen avatar and flips the row to member ownership between the read and the write. The sync would then overwrite `avatar_url` with the provider picture while `avatar_source` still reads `user`, and because every later sync sees a member-owned row and declines to write, that state is terminal: the member's chosen avatar is silently and permanently replaced. The claim that the sync self-corrects on the next login was incorrect — this is data loss, not a skipped refresh. With the predicate in the statement, the losing writer matches no row and changes nothing.

The costs that remain are one extra indexed read per login and one extra statement when the picture actually changed. The service still compares the incoming picture with the stored one, so an unchanged avatar issues no second statement at all and the common login path does not touch the column.

### Share the image-failure rule, not the avatar component

PrimeVue's `Avatar` exposes an `image` prop and emits `error` when that image fails to load, so the fallback is driven by the component's public contract rather than by reaching into its rendered DOM. Nothing in the component chooses initials on failure, so the fallback still has to be written, and it has to exist at both render sites.
