...
The redesigned auth screens introduced a `Browser extension` callout on the user login page — a puzzle-icon card reading "Track time right from your browser" with a forward arrow. It exists in the approved `.pen` screens but nowhere in the app, so the one moment a user is guaranteed to look at GiTiempo in a browser passes without telling them the Chrome extension exists. The same redesign changed the visual language of all four auth screens without any of it reaching `docs/ui`, which `apps/user-web` treats as the UI source of truth.

## What Changes

- Add the `Browser extension` callout to the user-web login page, below the sign-in action stack, matching the approved `Login Page` `.pen` screen.
