...
- [x] 5.4 Call `exportCurrentReport` with the active date range, the grouping's mapped API value (`project`/`user`), and the table's project and member filters
- [x] 5.9 Disable `Export CSV` while a search, hours, or billable filter is active, with the reason on a wrapper span since a disabled button swallows hover
...
- [x] 6.5 Assert the export payload carries the date range, grouping, and the table's project/member filters, and that an active search, hours, or billable filter blocks export entirely
