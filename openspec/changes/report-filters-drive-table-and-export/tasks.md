...
- [x] 5.4 Call `exportCurrentReport` with the active date range, the grouping's mapped API value (`project`/`user`), the table's project filter, and its member filter when grouping by member
- [x] 5.9 Disable `Export CSV` while a search, hours, or billable filter is active, or a member filter under `Project` grouping (folded rows show everyone's time; a member-scoped file would not); rule and wording live in `getReportExportBlockedReason`, reason on a wrapper span since a disabled button swallows hover
...
- [x] 6.5 Assert the export payload carries the date range, grouping, and the table's project/member filters under `Member` grouping; that search, hours, and billable block export; and that the member-filter-plus-`Project`-grouping combination blocks it too, with unit tests pinning `getReportExportBlockedReason` per grouping
