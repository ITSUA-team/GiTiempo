...
- Implement the missing Time Entries page record-management surface, including grouped entries, filters, pagination, manual-entry creation, edit dialog, and delete confirmation.
- Keep manual interval entry out of the top-bar timer and task-picker dialog; manual entry belongs only on Time Entries.
...
- `user-pages`: replace dedicated Timer page requirements with global top-bar timer behavior and complete Time Entries manual-entry/dialog requirements.
...
- Affected frontend app: `apps/user-web` shell, router, timer components/composables, Time Entries page, and tests.
...
- Affected UI docs/spec alignment: OpenSpec requirements must be updated to match `docs/ui/pages-user.md`, `docs/ui/layout.md`, `docs/ui/patterns.md`, and the approved `GITiempo.pen` frames `User Topbar Timer States` and `Top-Bar Timer Task Picker`.
