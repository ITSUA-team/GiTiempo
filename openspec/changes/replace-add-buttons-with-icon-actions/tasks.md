...
- [x] 1.1 Inspect the affected `apps/user-web` and `apps/admin-web` components that render Time Entries day headers, User Projects project headers, Admin Members table header, and Admin Projects table header; confirm Admin Invoices is deferred and excluded from visible icon-action scope.
...
- [x] 1.3 Build a parity checklist from `docs/ui/pages-user.md`, `docs/ui/pages-admin.md`, `docs/ui/patterns.md`, `docs/ui/components.md`, `docs/ui/accessibility.md`, and the `GITiempo.pen` frames `Time Entries`, `Projects List`, `Admin Members`, and `Admin Projects`; keep Admin Invoices hidden until invoice contracts exist.
...
- [x] 3.1 Defer Admin Invoices icon-action scope by hiding the temporary route content and shell navigation entry until an invoice API/contract exists.
...
- [x] 3.4 Update focused admin-web tests to query icon-only openers by accessible name for member invite and project new-route navigation, and to assert the deferred invoice route renders no temporary invoice table, action, or dialog.
