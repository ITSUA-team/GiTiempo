...
This change introduces TanStack Vue Query as the shared server-state layer for both SPAs and refactors large route composables into focused feature modules so page-level composition stays readable and behavior remains testable.
...
- Potential shared frontend code: small query-key factories, QueryClient test helpers, pure date/time formatters, and browser-only helpers may move to `packages/web-shared` only when behavior is proven identical across both SPAs.
