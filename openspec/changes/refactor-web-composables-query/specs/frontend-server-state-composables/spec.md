...
### Requirement: Web SPAs Use TanStack Query For Server State
The `user-web` and `admin-web` applications MUST use TanStack Vue Query as the ownership layer for frontend server-state reads and mutations while keeping existing API clients responsible for request transport, auth headers, payload shape, response parsing, and API error propagation.
...
- **WHEN** a Query-backed mutation succeeds
...
- **WHEN** a list surface such as time entries, visible projects/tasks, reports, members, or settings loads data from the backend
...
- **WHEN** a create, update, delete, start, stop, save, or export action changes data that other surfaces consume
