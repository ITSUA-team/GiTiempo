...
- [x] 2.1 Update `TasksService.listProjectTasks()` so default `GET /projects/:id/tasks` returns active tasks only for visible active projects according to the modified task-management spec.
- [x] 2.2 Add or update API tests covering active task inclusion, inactive task exclusion, and non-admin inactive-project visibility behavior.
- [x] 2.3 Run focused API verification for the affected tasks/projects tests and any required API lint/typecheck command.

## 3. User-Web Route And Client Boundary

- [x] 3.1 Update the user-web route map so the Projects list page is reachable at `/projects` through the authenticated shell.
- [x] 3.2 Update `AppShell.vue` Projects navigation to target the list route and keep active-state behavior correct.
- [x] 3.3 Extend the existing user-web task/projects client boundary with `updateTask(accessToken, taskId, input)` using `updateTaskSchema` and `taskResponseSchema`.
- [x] 3.4 Extend the existing user-web task/projects client boundary with `deleteTask(accessToken, taskId)` using the `204 No Content` endpoint contract and repository error-message parsing.
- [x] 3.5 Add focused client tests for task update payload/response parsing, delete no-content handling, and delete error propagation.

## 4. Projects Page State Module

- [x] 4.1 Build a focused Projects page state module or composable for loading visible projects, loading project tasks, tracking per-load errors, and retrying failed loads.
- [x] 4.2 Filter visible active projects and active tasks consistently with backend responses without relying on inactive task rows for default rendering.
...
- [x] 4.4 Implement dialog state for page-level create, project-level create with preselected project, edit prefill, validation errors, submit loading, request errors, and reset/close behavior.
...
- [x] 5.5 Implement the task create/update PrimeVue Dialog as a true popup with required project select, task title input, update status select, Cancel, Create task, and Save changes actions.
...
- [x] 6.1 Update router tests to cover `/projects` auth redirect behavior and authenticated route mounting.
- [x] 6.2 Add focused view/component tests proving header/search rendering, grouped sections, row action accessibility, and distinct loading/empty/error states.
- [x] 6.3 Add focused view/component tests for page-level create, project-level create preselection, edit prefill/save, delete confirmation success, and delete conflict behavior.
...
- [x] 7.5 Run the affected API verification from section 2 after frontend work remains green.
