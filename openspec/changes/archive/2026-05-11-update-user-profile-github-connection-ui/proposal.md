...
- Add GitHub connection actions for connect/reconnect and disconnect using the implemented API endpoints.
...
- Shared current-user/auth runtime helpers in `packages/web-shared` when extending the existing `PATCH /users/me` boundary.
...
- Matching `admin-web` verification when shared frontend helpers change.
