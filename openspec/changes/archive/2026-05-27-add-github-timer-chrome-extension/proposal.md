...
- Timer semantics: the injected control treats backend current-timer state as authoritative, restores inline `Stop Timer` only when the current-timer contract exposes stable GitHub issue linkage matching the current page, and otherwise avoids presenting an unlabeled global stop action from issue-page display text alone.
