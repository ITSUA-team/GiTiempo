...
- [x] 1.4 Get the `Ext Unauthenticated` frame approved before any popup markup changes, per `apps/chrome-ext/AGENTS.md`, which names the approved `.pen` extension frames as the source of truth for visual requirements. — **Provenance, recorded because the order was not clean.** The frame was drawn (1.3) and rendered for review before any markup existed. Markup then went ahead on an explicit instruction to continue, with formal sign-off still outstanding, and that gap was stated at the time rather than glossed over. Sign-off arrived on 2026-07-31, when the frames were accepted and the change cleared for merge. So the approval is real but it followed the markup; task 5.5's reference to "the frame approved in 1.4" means this frame and this approval.
...
- [x] 3.6 Confirm the extension target needs no change to `POST /auth/github/session`, or, if 2.3 applied, extend it there and nowhere else. — **Superseded.** This first recorded the endpoint as unchanged, which was true only while D5's primary branch held. 2.1 ruled that branch out, so 2.3 extended the endpoint with an optional `verifier` — required for a challenged handoff, omitted by web clients. The note is corrected rather than deleted so the sequence stays readable.
...
- [x] 5.4 Add an `exchangeGithubSession(code, verifier)` method to `src/lib/api.ts` beside `loginWithFirebaseToken`, reusing the existing `tokenPairResponseSchema` parsing and `setStoredSession` so the stored session shape is unchanged.
