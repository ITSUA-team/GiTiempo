...
### Requirement: User Page API Feedback Uses Safe Toast Copy

User-web pages MUST show safe product-controlled copy in global API error toasts while preserving backend error details for diagnostics and scoped feature behavior.

#### Scenario: Profile API failure toast avoids raw backend copy

- **WHEN** a Profile page identity or GitHub connection API action fails
- **THEN** the page shows an error toast with action-specific safe copy
- **AND** the page does not display the raw backend error message in the toast detail
- **AND** the original error is logged for diagnostics

#### Scenario: Profile callback toasts use safe mapped copy

- **WHEN** the Profile page handles GitHub callback query outcomes
- **THEN** callback toasts use product-controlled success or error copy
- **AND** unknown callback error codes show generic safe copy rather than rendering the code as backend-provided text
...
- **THEN** the page shows an error toast with safe product-controlled copy rather than the raw API failure message
- **AND** the page logs the backend failure details for diagnostics
...
- **THEN** failed API calls show an error toast with safe action-specific copy rather than raw backend/API error messages
- **AND** backend/API failure details remain available for logging, inline scoped feature state, and recovery logic
...
- **THEN** the page shows an error toast with safe product-controlled copy rather than the raw API failure message
- **AND** the page logs and preserves the backend failure detail for conflict detection
