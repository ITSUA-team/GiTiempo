...
- [ ] 3.2 Give `findGitHubProjectRef` a deterministic order so an existing duplicate cannot resolve differently between requests
- [ ] 3.3 Add a service test pinning that order, so the guarantee is not lost to a later refactor

## 4. Refuse duplicate project names

- [ ] 4.1 Check the conflict in the project create path and return a typed conflict naming the collision, compared case-insensitively among active projects
- [ ] 4.2 Check the same on project update, allowing a project to keep or recase its own name
- [ ] 4.3 Confirm disabling a project releases its name for reuse
- [ ] 4.4 Add API tests for create, rename, self-rename, recase, and the disabled-project case

## 5. Surface the conflicts to users

- [ ] 5.1 Add the conflict shape to the shared project contracts so both web apps read one definition
- [ ] 5.2 Report the name conflict against the Project name field on the Add Project form, keeping entered values intact
- [ ] 5.3 Report a colliding derived name for a GitHub import as a name conflict rather than an unimportable project
- [ ] 5.4 Report the conflict on the project edit form
- [ ] 5.5 Add component tests covering the conflict message on create, on import, and on rename

## 6. Verification

- [ ] 6.1 Run lint, typecheck and tests for `@gitiempo/api`, `@gitiempo/shared`, `user-web`, and `admin-web`
- [ ] 6.2 Confirm the extension needs no release, since normalization is server-side
