...
Link 3 is a known defect that was already fixed once, for the neighbouring table. Migration `0015_dedup_github_issue_casing` collapsed exactly this split in `task_external_refs` and rebuilt its unique index on `lower(external_key)`, and the schema carries a comment explaining why. `project_external_refs` never received the same treatment, so the mirror table still keys uniqueness on the raw string. This change finishes that job rather than inventing a new approach.
...
- Every path that creates a repository mapping records the identifier GitHub reports, instead of the caller's spelling. The timer-from-issue path is the one currently violating this: it stores what the caller sent, and the caller is any client, not only the extension.
- Project-mapping uniqueness becomes case-insensitive, matching what `task_external_refs` already does since migration `0015`, so the database refuses a second mapping for a repository that already has one. This is what the data model already claims under *Provider lookup remains unique within workspace*; today the guarantee holds only for byte-identical strings.
- Existing duplicate mappings are resolved by migration before the new index can be created. One mapping survives per repository, chosen by the same rule `0015` used: the row carrying GitHub's real casing, then the older project. The duplicate mapping is dropped, and **the project it pointed at is left in place, unlinked and reported** — unlike `0015`, which could safely delete a duplicate task, deleting a duplicate project would take its assignments and settings with it and cascade away the GitHub issue links of any task moved off it.
...
2. **Uniqueness follows `0015`, not a GitHub-only variant — decision taken.** An earlier draft proposed a partial index scoped to `provider = 'github'`, reasoning that another provider's identifiers might be case-sensitive. The team already settled that question the other way for `task_external_refs`, and two mirror tables with different identity models would be worse than either choice on its own.
3. **A duplicate project is unlinked, never deleted — decision taken.** `0015` could delete a duplicate task because only `time_entries` and its own refs pointed at it. A project is pointed at by `tasks`, `project_assignments`, `project_external_refs`, and `task_external_refs.project_id`, the last two on cascade, so deleting one would drop assignments and destroy issue links. The migration therefore stops at unlinking and reports what a human has to decide.
4. **Name uniqueness is scoped to the workspace and to active projects — for review.** Global uniqueness would let one workspace reserve a name against every tenant; counting archived projects would reserve a name forever after a project is disabled, which contradicts soft-disable semantics.
5. **Name uniqueness does not fix the observed duplicate — stated so nobody relies on it.** The two projects' names differ, so only the relation check and the mapping constraint prevent that case. The name rule is included as a product rule in its own right.
...
- `data-model`: project-mapping uniqueness becomes case-insensitive, matching the rule task mappings already follow; active project names become unique per workspace.
...
- **Migration**: hand-written, following `0015_dedup_github_issue_casing`. Drop duplicate project mappings by the same selection rule, leave the unlinked projects in place and report them, then rebuild the mapping unique index on `lower(external_key)`. Duplicate active names are reported and stop the deploy; they are never auto-renamed.
- **Extension**: no contract change. `0015` records that the extension already stopped lowercasing, so this is not about one client — the backend simply must not trust any caller's spelling.
