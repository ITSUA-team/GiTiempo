...
- The matching `chrome-extension://<extension-id>` origin must be present in `ALLOWED_ORIGINS`, or the session exchange that follows the redirect fails on CORS even though the OAuth flow itself succeeded.
