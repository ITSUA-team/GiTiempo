...
**New workspace registration:** The User SPA may expose a `/register` flow for creating the first owner account of a new workspace only after an approved backend/API registration contract exists. Until that contract is approved and implemented, there is no public registration endpoint, the route must remain unregistered or intentionally gated, and the UI must not ship as a disabled placeholder. This first-owner flow is separate from invite acceptance; it must not replace the invite-only flow for members joining an existing workspace.
