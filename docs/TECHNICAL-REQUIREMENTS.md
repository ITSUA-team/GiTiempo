...
**Invite accept page flow:** Invite emails link to the User SPA at `/invites/accept?token=<invite-token>`. This route is unauthenticated and outside the authenticated app shell. The page supports self-service Firebase email/password account creation for invitees who do not want to use Google and who do not already have a Firebase account. In the default create-account path, the invitee enters the invited email, a new password, and password confirmation; the frontend creates the Firebase account with the Firebase client SDK, reads the resulting Firebase ID token, submits `POST /invites/accept` with the invite token and Firebase ID token, then creates the normal app API session with that same Firebase ID token after invite acceptance returns `204 No Content`. Existing invitees can switch to the sign-in path and authenticate with Firebase email/password or Google, then use the same `POST /invites/accept` flow. Missing, expired, reused, or unknown invite tokens are terminal link errors; email mismatch is retryable with the correct identity. If Firebase account creation succeeds but invite acceptance fails, the page must keep the user on a retry/recovery state and explain whether to retry acceptance, switch accounts, or request a fresh invite; the backend must remain the source of truth for invite validity and membership creation.
