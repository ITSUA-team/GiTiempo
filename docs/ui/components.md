...
- Use column-appropriate PrimeVue filter controls: `InputText` for text/id/name/email/source filters; `Select` for single-choice filters when the possible option set is 5 items or fewer; `AutoComplete dropdown forceSelection` for single-choice filters when the option set can exceed 5 items; the shared `ManagementTableAssignmentFilter` (`AutoComplete multiple dropdown forceSelection`) for admin management assignment filters such as Members table `Projects Assigned` and Projects table `Assigned members`; `MultiSelect filter display="chip"` only for other assignment filters that have not adopted the shared management-table control; and numeric/date controls only when the column data type requires them.
