...
- The page has no report setup bar. The results table header carries the date range, global search, and an `Export ▾` menu action (brand button with download icon, label, hairline divider, and chevron) whose menu offers `Export as CSV` and `Export as PDF` — the PDF item carries a brand-colored file icon and the subtitle `Styled summary & totals` — in that order; the grouping builder sits on its own full-width row directly below the header (source: the approved "Admin Reports V2" `.pen` frame and the `report-grouping` OpenSpec change).
- The date control is the existing PrimeVue `<DatePicker selectionMode="range" :manualInput="false">` at `h-[38px]`; report presets persist only its concrete absolute `dateRange`.
- A saved reports bar sits above the summary cards (source: the approved "Admin Reports V2" `.pen` frame, node `savedReportsBar`): a `Saved reports` label, one 32px pill per preset with the loaded preset tinted `accent-tint` and bookmark-marked, a `+ New report` pill, and on the right an unsaved-changes dot, a `Save` button, and a `Save as new…` action.
- Presets are shared across the workspace and any admin or PM can edit or delete any of them; names are unique per workspace, ignoring case and surrounding whitespace. A preset stores filter selections only, so opening one never widens a PM's report scope — two users may open the same preset and see different rows.
- A preset captures the absolute date range, the ordered grouping path, the project and member scope, and every column filter. Opening one restores all of it and reloads the report. The grouping path may include a `Billable` level — this is additive, so presets saved before the dimension existed keep working, and a stored dimension a reader does not recognise is dropped on load.
- `Save` overwrites the loaded preset and is disabled until the view actually differs from it. `Save as new…` prompts for a name and surfaces a duplicate name inline without creating anything. `New report` clears the loaded preset and resets grouping, scope, and column filters, deliberately preserving the current date window.
- Rename and delete live in an overflow menu on the loaded preset's pill. This is a deliberate addition to the approved frame, which shows no way to remove a preset.
- A preset naming a project or member the viewer can no longer see falls back to the unfiltered choice, reports the fallback, and still loads the report.
- Grouping is an ordered path of one to four unique levels drawn from `Project`, `Member`, `Task`, and `Billable` (default: a single `Project` level). `Billable` is not an entity dimension: it splits each level it sits under into a `Billable` and a `Non-billable` bucket by whether the entries are billable. The builder renders one accent-tint pill chip per level with a grip icon, label, and remove `×` (the last remaining level cannot be removed), `pi-chevron-right` separators between chips, a `+ Add level` select offering only unused dimensions, and a `Drag to reorder` hint. Chips reorder via native HTML drag-and-drop.
...
- Results table column filters use the existing management-table filter-row treatment, and every column carries one: project and member autocomplete selects, plus preset selects for hours (Any / Tracked / 8h+ / 40h+), billable (Any / Billable / Non-billable), billable share (Any / Below 50% / 50%+ / 90%+), and last activity (Any time / Today / Last 7 days / Last 30 days). The aggregate filters compare the displayed totals of top-level groups (never the invisible leaves underneath), keep qualifying groups' whole subtrees, and the total row follows the visible groups; a group with no billable share or no activity only passes the Any option.
- Export mirrors the on-screen report (WYSIWYG): both CSV and PDF are built from the filtered, grouped report tree currently shown — the same rows, per-level subtotals, and total — so the file always matches the table, including every active search and column filter.
- CSV export is serialized in the browser from that on-screen tree; no backend export request is made. Workspace-controlled names (projects, members, tasks) are guarded against spreadsheet formula injection.
- PDF export builds the same document in the browser and POSTs it to `/reports/time/export/pdf`, where the backend only applies the approved "Report PDF Preview" styling (brand masthead, period and workspace, filters and grouping line, summary strip, the grouped table at the current grouping path with per-level subtotals, a total row, and footers with the generation date and page numbers) without re-querying report data. It renders in the standard Helvetica family (the design's Inter webfont is not embedded).
- Because the file reflects the screen, no filter blocks the export: search, the aggregate column filters (hours, billable, billable share, last activity), and the project/member selects are all carried into the file. The `Export` menu stays enabled whenever the date range is valid.
- Both formats follow the current grouping path and carry the grouped, subtotaled tree exactly as displayed, never detailed project-task-user rows. There is no backend report-export endpoint beyond the PDF styler; the old `/reports/time/export` CSV/PDF pipeline was removed when the export moved client-side.
...
- The card includes an `Allowed organizations` section with one row per saved GitHub organization login. Each row shows the organization login, helper copy `Allowed for this workspace`, and a `Remove` action. After installation status loads successfully, show a small outlined secondary `Install App` button beside `Remove` when no verified association exists, including absent, suspended, unavailable, and locally disconnected states. Hide it while status is unknown or failed, and for verified associations. Keep the action group responsive without installation status labels or a separate installations card.
- When the policy list loads, and after an organization is successfully added, the client automatically attempts to verify the configured GitHub App for each allowed organization that has no saved installation association. It requires the current admin's usable personal GitHub connection and active organization-owner authority.
- This automatic attempt creates opaque one-time setup state, discovers an existing installation using an App JWT, and completes the existing full owner, installation-visibility, App, organization, and permission checks only when discovery returns a candidate. It never redirects the administrator to GitHub: a GitHub 404 leaves the association absent, and another discovery failure leaves the policy UI unchanged. Existing verified, suspended, unavailable, and locally disconnected associations are not retried or reactivated automatically.
- GitHub callback values are sent only to the authenticated completion endpoint and removed from the URL afterwards. They do not change the Workspace Access card layout or expose installation status.
- An explicit `Install App` click starts authenticated setup. An existing installation candidate completes full verification without navigation; successful setup without a candidate navigates in the current tab to the configured GitHub App installation page with opaque state. GitHub handles account selection, and completion rejects an installation for the wrong organization or a personal account. Authority, configuration, and discovery failures report existing toast feedback without navigation. Pending manual setup shows loading on its row and disables additional Install App clicks. Without a usable personal connection, the action reports prerequisite guidance.
- Explicit setup can restore a non-verified saved association only after all checks pass; background discovery never restores saved associations. Successful completion refreshes installation status and hides the button. The button's label does not assert that the App is absent on GitHub.
- Completion returns HTTP 409 with `Reconnect GitHub, then retry installation verification` when successful provider lookups show that the exact installation is inaccessible to the administrator's token. When installation access is confirmed but active owner membership or the expected organization identity does not match, it returns HTTP 403 with `GitHub organization owner authority is required`. The administrator can reconnect from the user profile and retry with fresh setup state; reconnection is not a guaranteed remedy or a requirement for tracking members.
...
- These recovery cards apply only to adding an allowed organization with the administrator's personal user-to-server connection. Installation-backed tracking recovery is handled server-side and does not add status or actions to the Workspace Access rows.
- External GitHub links in the organization-add recovery checklist open in a new tab and use clear accessible labels such as `Open GitHub App install page for My-test-org-for-clock`; internal reconnect links keep the current app's standard navigation treatment. The allowed-row `Install App` setup action uses the current tab to carry its setup-and-return flow.
