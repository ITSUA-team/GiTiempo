...
- The page has no report setup bar. The results table header carries a relative-period select, the date range, global search, and an `Export ▾` menu action (brand button with download icon, label, hairline divider, and chevron) whose menu offers `Export as CSV` and `Export as PDF` — the PDF item carries a brand-colored file icon and the subtitle `Styled summary & totals` — in that order; the grouping builder sits on its own full-width row directly below the header (source: the approved "Admin Reports V2" `.pen` frame and the `report-grouping` OpenSpec change).
- The date controls pair a relative-period `Select` — `Custom range` plus `This week`, `This month`, `Previous month`, `Last 7 days`, `Last 30 days` — beside the existing `<DatePicker selectionMode="range" :manualInput="false">` range picker at `h-[38px]`, with the search field and export button.
...
- Selecting a relative period resolves it to a concrete window in the viewer's local calendar and fills the range picker; editing the dates by hand (or choosing `Custom range`) clears the period. A preset stores the active relative period — re-resolving it to the current window every time the preset is opened — or the pinned absolute window when the range was picked by hand.
