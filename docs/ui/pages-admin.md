...
- A preset stores its date window as picked. The contract also supports relative periods that re-resolve on open, but no control currently selects one, so presets save absolute windows until a period control ships.
