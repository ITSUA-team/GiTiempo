...
- A preset stores its date window as picked. The date-range contract still supports relative periods that re-resolve on open — a loaded relative preset restores and re-resolves its period — but there is no in-page control to pick one, so a preset built in the UI saves its absolute window.
