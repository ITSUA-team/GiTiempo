...
- A report period select sits beside the date picker offering `This week`, `This month`, `Previous month`, `Last 7 days`, `Last 30 days`, and `Custom range`. Picking a period resolves it against the viewer's local calendar and fills the date picker; editing the picker by hand clears the period back to `Custom range`. The period exists because saved reports store the *intent* rather than the resolved dates — see the saved reports bullets below.
...
- A preset captures the report period (relative or a pinned absolute window), the ordered grouping path, the project and member scope, and every column filter. Opening one restores all of it and reloads the report.
- Relative periods re-resolve on open, so a preset named `Monthly billing` reports the current month every month. Dirty state therefore compares the stored shape, never the resolved window — a relative preset does not read as changed simply because time passed.
