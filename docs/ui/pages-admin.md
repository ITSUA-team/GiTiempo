...
- CSV export downloads backend-generated detailed project-task-user rows for the current date range, and also carries the table's project and member filters so the file is scoped to the same projects and members as the table.
- Search, hours, and billable filters cannot scope the export. Hours and billable filter aggregate row totals, and the global search matches formatted labels including durations and percentages; the CSV is detailed project-task-user rows holding none of those. The export endpoint's own `search` is not an equivalent either: it matches task titles the table never shows and ignores the duration labels the table does, so sending it would apply a different filter under the same name. While any of the three is active, `Export CSV` is disabled with a tooltip saying so.
